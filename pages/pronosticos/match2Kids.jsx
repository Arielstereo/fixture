...
export default function Stage1({ match2Kids }) {
  ...
  const data = async (result) => {
    try {
      const res = await fetch("/api/match2Kids", {
        method: "POST",
        headers: {
  ...
  const handleSubmit = async (e) => {
    ...
    const duplicate = await match2Kids.find(
    ...
    if (
      ...
      router.push("/results/resultsMatch2Kids");
    }
  };
...
export async function getServerSideProps() {
  try {
    ...
    const match2Kids = res.map((item) => {
      const user = item.toObject();
      user._id = item.id.toString();
      return user;
    });
    return { props: { match2Kids } };
  } catch (error) {
    console.log(error);
  }
}
