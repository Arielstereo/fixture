...
export default function Finals4({ cuartos }) {
  const {
    name,
    surname,
    password,
  ...
  const handleSubmit = async (e) => {
    ...
    const duplicate = await cuartos.find(
    ...
    if (
      ...
      router.push("/results/resultsCuartos");
    }
  };
...
export async function getServerSideProps() {
  try {
    ...
    const cuartos = res.map((item) => {
      const user = item.toObject();
      user._id = item.id.toString();
      return user;
    });
    return { props: { cuartos } };
  } catch (error) {
    console.log(error);
  }
}
