...
export default function Finals4({ finalsKids }) {
  const {
    name,
    surname,
    password,
  ...
  const data = async (result) => {
    try {
      const res = await fetch("/api/finalsKids", {
        method: "POST",
        headers: {
  ...
  const handleSubmit = async (e) => {
    ...
    const duplicate = await finalsKids.find(
    ...
    if (name && surname && password && checked && checked2 && !duplicate) {
      ...
      router.push("/results/resultsFinalsKids");
    }
  };
        ...
        <div className="flex flex-col text-center pt-32 pb-4 gap-4">
          <div className="flex flex-col gap-4">
            <h3 className="text-sky-400 font-bold text-4xl md:text-6xl py-4">
              Final y tercer puesto <span className="text-yellow-400">Kids</span>
            </h3>
...
export async function getServerSideProps() {
  try {
    await dbConnection();
    const res = await FinalsKids.find({});
    const finalsKids = res.map((item) => {
      const user = item.toObject();
      user._id = item.id.toString();
      return user;
    });
    return { props: { finalsKids } };
  } catch (error) {
    console.log(error);
  }
}
