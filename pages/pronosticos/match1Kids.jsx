...
export default function Stage1({ match1Kids }) {
  const {
    name,
    family,
    setFamily,
  ...
  const data = async (result) => {
    try {
      const res = await fetch("/api/match1Kids", {
        method: "POST",
        headers: {
  ...
  const handleSubmit = async (e) => {
    ...
    const duplicate = await match1Kids.find(
    ...
    if (
      ...
      router.push("/results/resultsMatch1Kids");
    }
  };
...
export async function getServerSideProps() {
  try {
    ...
    const match1Kids = res.map((item) => {
      const user = item.toObject();
      user._id = item.id.toString();
      return user;
    });
    return { props: { match1Kids } };
  } catch (error) {
    console.log(error);
  }
}
